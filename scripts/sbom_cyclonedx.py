#!/usr/bin/env python3
"""SBOM CycloneDX 1.5 minimal (JSON) des paquets Python installés, sans dépendance externe.
...
def main():
    ...
    bom = {"bomFormat": "CycloneDX", "specVersion": "1.5", "serialNumber": f"urn:uuid:{uuid.uuid4()}", "version": 1,
           "metadata": {"timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(), "tools": [{"name": "sbom_cyclonedx.py (kit pré-commit conformité dev)"}]},
           "components": comps}
    os.makedirs(os.path.dirname(a.out) or ".", exist_ok=True)
    with open(a.out, "w") as f: json.dump(bom, f, indent=2)
    print(f"SBOM : {len(comps)} composants → {a.out}")
